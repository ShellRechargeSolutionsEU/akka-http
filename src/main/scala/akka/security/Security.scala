...
class AkkaSecurityFilterFactory extends ResourceFilterFactory {
  ...
  lazy val authenticatorFQN = {
    ...
    if (auth == "N/A") throw new IllegalActorStateException("The akka.config option 'akka.http.authenticator' is not defined in 'akka.conf'")
    auth
  }
 ...
 * Don't forget to set the authenticator FQN in the rest-part of the akka akka.config
 */
 ...
 * rest-part of the akka akka.config
 */
...
trait SpnegoAuthenticationActor extends AuthenticationActor[SpnegoCredentials] {
  ...
  lazy val servicePrincipal = {
    ...
    if (p == "N/A") throw new IllegalActorStateException("The akka.config option 'akka.http.kerberos.servicePrincipal' is not defined in 'akka.conf'")
    p
  }
  ...
  lazy val keyTabLocation = {
    ...
    if (p == "N/A") throw new IllegalActorStateException("The akka.config option 'akka.http.kerberos.keyTabLocation' is not defined in 'akka.conf'")
    p
  }

  lazy val kerberosDebug = {
    val p = Config.config.getString("akka.http.kerberos.kerberosDebug", "N/A")
    if (p == "N/A") throw new IllegalActorStateException("The akka.config option 'akka.http.kerberos.kerberosDebug' is not defined in 'akka.conf'")
    p
  }
  ...
  /**
   * this class simulates a login-akka.config.xml
   */
